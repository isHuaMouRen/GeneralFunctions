...
namespace ToolLib.ErrorReportLib
{
    public class ErrorReportBox
    {
        ...
        public static DialogResult Show(string title, string text, Exception ex, bool showCloseButton = true)
        {
        ...
        /// <param name="showCloseButton">是否显示关闭按钮</param>
        /// <returns>错误提示框的返回值，默认DialogResult.Cancel、关闭按钮DialogResult.Abort、继续按钮DialogResult.OK</returns>
        public static DialogResult Show(string title, string text, string ex, bool showCloseButton = true)
        {
            return Show(title, text, new Exception(ex), showCloseButton);
        }
    }
}
